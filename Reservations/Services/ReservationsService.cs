namespace Reservations.Services
{
    ...
    public class ReservationsService : IReservationsService
    {
        ...
        public ValidationResult ValidateNewReservation(NewReservationItem newReservation)
        {
            ...
            if ((newReservation.From.Hour < 8 || newReservation.From.Hour >= 18) || (newReservation.To.Hour < 8 || newReservation.To.Hour > 18))
            {
            ...
            var totalDuration = newReservation.To.Hour - newReservation.From.Hour;
            if (totalDuration > 3)
            {
            ...
            List<Reservation> ReservationList = _queryAll.Execute().Where(m => m.From.Date == newReservation.From.Date && m.Hall.Number == newReservation.LectureHallNumber).OrderBy(x => x.From.Date).ToList();

            //if (_queryAll.Execute().Where(m =>  && && m.From.Hour == newReservation.From.Hour).Count() > 0)
            //{
            //    result = ValidationResult.Conflicting;
            //}
            if (_queryAll.Execute().Where(m => m.Hall.Number == newReservation.LectureHallNumber && m.Lecturer.Id==newReservation.LecturerId  && m.From.Date == newReservation.From.Date && (m.From.Hour == newReservation.From.Hour || m.To.Hour == newReservation.To.Hour || (newReservation.From.Hour >= m.From.Hour && newReservation.From.Hour <= m.To.Hour) )).Count() > 0)
                        {
                result = ValidationResult.Conflicting;
            }
            ...
            if (_queryAllLectureHalls.Execute().Where(m => m.Number == newReservation.LectureHallNumber).Count() == 0)
            {
            ...
            if (_queryAllLecturers.Execute().Where(m => m.Id == newReservation.LecturerId).Count() == 0)
            {
